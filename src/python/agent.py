...
class SimpleAgent(Agent):
    ...
    def _get_part_of_map(self, orientation, ortho_orientation, distance, width): 
        ...
        if (x_min_ind >= 0) and (x_max_ind <= len_x_ind) and (y_min_ind >= 0) and (y_max_ind <= len_y_ind):
            ...
            coarse_cut_mask = np.ogrid[y_min_ind:y_max_ind, x_min_ind:x_max_ind]
            ...
            ground_map_cut = np.ones((y_max_ind-y_min_ind, x_max_ind-x_min_ind)) * BACKGROUND
            ...
            coarse_cut_mask = np.ogrid[y_min_ind_cut:y_max_ind_cut, x_min_ind_cut:x_max_ind_cut]
            ...
            ground_map_cut[zero_shift_y:len_cut_y,zero_shift_x:len_cut_x] = ground_map[tuple(coarse_cut_mask)].copy()
...
class RayAgent(Agent):
    ...
    def _get_part_of_map(self, orientation, ortho_orientation, distance, width): # TODO finish implementation with angles
        ...
        if orientation in ([1,0], [-1,0], [0,+1], [0,-1]):
            ...
            ymask, xmask = np.ogrid[0:ground_map_cut.shape[0], 0:ground_map_cut.shape[0]]

            # ((xmask-corner_ymax[0]) * (corner_xmax[1] - corner_ymax[1]) / (corner_xmax[0] - corner_ymax[0]) + corner_ymax[1])
            bot_right_mask = self.line_connection(xmask, corner_ymax, corner_xmax) < ymask
            bot_left_mask = self.line_connection(xmask, corner_xmin, corner_ymax) < ymask
            top_right_mask = self.line_connection(xmask, corner_ymin, corner_xmax) > ymask
            top_left_mask = self.line_connection(xmask, corner_xmin, corner_ymin) > ymask

            corner_masks = [bot_right_mask, bot_left_mask, top_right_mask, top_left_mask]
            for corner_mask in corner_masks:
                mask[corner_mask] = False
            
            # [mask[corner_mask] = False for corner_mask in corner_masks]
