...
class DynamicObject(Obstacle):
    ...
    def __init__(self, Shape, ControllingAgent, initial_position, initial_velocity, initial_acceleration, ID=None):
        ...
        self.steering_angle = -1 #Shape.calc_rot_angle() * 180 / np.pi
    ...
    def throttle_and_angle_to_acceleration(self, throttle): # TODO finetune
        """
        Transform action["throttle"] to accelertation taking orientation into account
            [-1, 1] -> ([-6.5 to 6.5], [-6.5 to 6.5]) 
        """

        angle = self.Shape.calc_rot_angle() + self.steering_angle * np.pi / 180

        orientation = self.Shape.calc_orientation_from_angle(angle) 
        self.Shape.orientation = self.Shape.calc_orientation_from_angle(angle)
        orthogonal_orientation = self.Shape.orthogonal_orientation()

        # TODO check, why does 10.06 work so well for timestep=0.1, angle = 1 deg? squared velocity wrong?
        self.acceleration = orientation * throttle * 6.5 - orthogonal_orientation *10.07* np.linalg.norm(self.velocity)**2 * np.tan(self.steering_angle * np.pi / 180) 
        print(orthogonal_orientation)
    ...
    def update_state(self, action, timestep): # TODO pyhscally correct update cycle...?
        ...
        self._update_acceleration(action)
    ...
    def _update_acceleration(self, action):
        ...
        self.throttle_and_angle_to_acceleration(throttle)
        print('DONE acc')
...
class ThreeDegFreedomObject(DynamicObject):
